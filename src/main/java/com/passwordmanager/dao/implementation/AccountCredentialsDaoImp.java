...
public class AccountCredentialsDaoImp implements AccountCredentialsDao {
	...
	public boolean addAccount(AccountCredentials account) {
		...
		try (Connection conn = ConnectorFactory.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			if (e.getErrorCode() == 1) {
				logger.warn("Failed to add account '{}', already exists for userId={}", account.getAccountName(),
						account.getUserId());
				return false;
			}
	...
	public boolean updateAccount(AccountCredentials account) {
		...
		try (Connection conn = ConnectorFactory.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			logger.error("DB error while updating account '{}' (accountId={})", account.getAccountName(),
