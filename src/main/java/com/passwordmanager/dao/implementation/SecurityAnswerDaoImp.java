...
public class SecurityAnswerDaoImp implements SecurityAnswerDao {
	...
	public boolean addAnswer(SecurityAnswer answer) {
		...
		try (Connection conn = ConnectorFactory.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			if (success) {
				logger.info("Added security answer for userId={} questionId={}", answer.getUserId(),
						answer.getQuestionId());
			}
			return success;

		} catch (SQLException e) {
			logger.error("DB error while adding security answer for userId={} questionId={}", answer.getUserId(),
					answer.getQuestionId(), e);
	...
	public boolean updateAnswer(SecurityAnswer answer) {
		...
		try (Connection conn = ConnectorFactory.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			boolean success = ps.executeUpdate() > 0;
			if (success) {
				logger.info("Updated security answer for userId={} questionId={}", answer.getUserId(),
						answer.getQuestionId());
			} else {
				logger.warn("Failed to update security answer for userId={} questionId={}", answer.getUserId(),
						answer.getQuestionId());
			}
			return success;

		} catch (SQLException e) {
			logger.error("DB error while updating security answer for userId={} questionId={}", answer.getUserId(),
					answer.getQuestionId(), e);
	...
	public boolean deleteAnswer(int userId, int questionId) {
		...
		try (Connection conn = ConnectorFactory.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			if (success) {
				...
				logger.warn("Attempted to delete non-existing security answer for userId={} questionId={}", userId,
						questionId);
			}
			return success;
