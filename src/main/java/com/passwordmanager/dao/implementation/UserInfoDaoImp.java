...
public class UserInfoDaoImp implements UserInfoDao {
	...
	public boolean createUser(UserInfo user) {
		...
		try (Connection conn = ConnectorFactory.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			if (e.getErrorCode() == 1) {
				logger.warn("Failed to create user '{}', email '{}' already exists", user.getUserName(),
						user.getUserEmail());
				return false;
			}
	...
	public boolean updateUser(UserInfo user) {
		...
		try (Connection conn = ConnectorFactory.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			boolean success = ps.executeUpdate() > 0;
			if (success) {
				logger.info("Updated user info for userId={}: new username='{}', new email='{}'", user.getUserId(),
	...
	public boolean verifyLoginByUsernameOrEmail(String input, String password) {
		...
		try (Connection con = ConnectorFactory.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
